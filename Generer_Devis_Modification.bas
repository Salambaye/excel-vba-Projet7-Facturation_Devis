...
Sub AppliquerBorduresTableau(ligneDebut As Long, ligneFin As Long)
    ' Cette fonction applique les bordures uniquement à l'extérieur du tableau et entre les colonnes (bordures verticales internes)
...
'========================================================================================
' MODULE DEVIS MODIFICATION - CORRECTION COMPLÈTE
'========================================================================================
...
'    End If
'
'    ' CORRECTION: Ajouter la description AVANT l'en-tête
''    With wsDevis
''        .Cells(ligneActuelle, 1).Value = Description
''        .Cells(ligneActuelle, 1).Font.Bold = True
''        .Cells(ligneActuelle, 1).Font.Size = 16
''        .Cells(ligneActuelle, 1).Font.Name = "Arial"
''        .Cells(ligneActuelle, 1).Font.Color = RGB(30, 58, 138)
''        .Cells(ligneActuelle, 1).WrapText = True
''
''        ' Fusionner sur toute la largeur pour que le texte se wrap correctement
''        .Range(.Cells(ligneActuelle, 1), .Cells(ligneActuelle, 6)).Merge
''        .Cells(ligneActuelle, 1).WrapText = True
''
''        .Rows(ligneActuelle).RowHeight = 60  ' Hauteur adaptative
''    End With
''
''    ' CORRECTION: Passer à la ligne suivante APRÈS la description
''    ligneActuelle = ligneActuelle + 1
'
'    ' Créer les en-têtes du tableau à la ligne suivante
'    Call CreerEntetesTableauModification(ligneActuelle)
'
'    ' CORRECTION: Commencer les données après l'en-tête
'    ligneActuelle = ligneActuelle + 1
'
'    ' Variables pour les totaux
'    Dim totalHT As Double
...
'    ' Compléter le tableau jusqu'à 15 lignes minimum
'    Dim ligneFinTableau As Long
'    ligneFinTableau = ligneDebut + 17  ' description(1) + en-tête(1) + 15 lignes de contenu
'
'    ' Compléter avec des lignes vides si nécessaire
'    Do While ligneActuelle < ligneFinTableau
'        With wsDevis
...
'    Loop
'
'    ' Appliquer les bordures au tableau complet (SANS la description)
'    Call AppliquerBorduresTableau(ligneDebut + 1, ligneFinTableau)
'
'    ' Calcul du total TTC
'    totalTTC = totalHT + montantTVA
'
...
'            .Cells(ligne, 6).Value = Format(montantTTC, "#,##0.00") & " €"
'
'            ' Mise en forme (sans bordures, appliquées plus tard)
'            With .Range(.Cells(ligne, 1), .Cells(ligne, 6))
'                .Font.Name = "Arial"
'                .Font.Size = 20
'                .VerticalAlignment = xlCenter
...
'        .Borders(xlInsideVertical).Weight = xlThin
'
'        ' Bordure horizontale uniquement sous l'en-tête
'        wsDevis.Range(wsDevis.Cells(ligneDebut, 1), wsDevis.Cells(ligneDebut, 6)).Borders(xlEdgeBottom).LineStyle = xlContinuous
'        wsDevis.Range(wsDevis.Cells(ligneDebut, 1), wsDevis.Cells(ligneDebut, 6)).Borders(xlEdgeBottom).Weight = xlMedium
'    End With
'End Sub
